...
You can also use pipes `|` to separate groups of commands to be processed. For example, `defisheye=1|invert`.

To use the bot on Discord, prefix your command with "(yourprefix)". An empty command or "random" selects random parameters.
...
| `swapuv`        | `uv`        | -      | -    | -            | Adds a swapuv effect on the video                                                                   |
| `defisheye`     | `defe`      | Number | 1    | 2            | Adds a defisheye effect on the video                                                                |
| `wave`          | `wav`       | Number | -600 | 600          | Adds a wave to the video; higher values means the wave scrolls faster.                              |
| `waveamount`    | `wava`      | Number | 1    | 100          | Controls amount of waves.                                                                           |
| `wavestrength`  | `wavs`      | Number | 1    | 400          | Controls how big waves are                                                                          |
...
| `wobble`        | `wub`       | Number | 1    | 1000         | Makes the audio wobbly.                                                                             |
| `bass`          | `bs`        | Number | 0    | 100          | Bass boost                                                                                          |
| `pitch`         | `pch`       | Number | 100  | 100          | Sets the audio pitch to be higher or lower                                                          |
| `reverb`        | `rv`        | Number | 0    | 100          | Adds a reverb, or echo, effect                                                                      |
...
| `music`         | `mus`       | Text   | -    | -            | (Requires yt-dlp)                                                                                    |
...
| `sfx`           | `sfx`       | Number | 1    | 100000       | Adds random sound effects                                                                           |
...
| `ricecake`      | `rc`        | Number | 1    | 696          | Clones delta frames (corrupting video) and clones audio                                             |
